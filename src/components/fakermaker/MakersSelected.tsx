...
export const MakersSelected = React.memo(({selectedMakers, deleteSelectedMaker}: ActiveProps) => {
    console.log("MakersSelected render, selectedMakers:", selectedMakers);

    if (!selectedMakers || selectedMakers.length === 0) {
        return <div>No makers selected</div>;
    }

    return (
        <div>
            {selectedMakers.map((maker, index) => (
                <div key={maker.nickName || index} className="group inline-block cursor-pointer bg-accent_pink border-2 border-transparent active:bg-subtle_blue relative text-sm font-semibold transition duration-150 text-nowrap mr-7 mb-5 px-[18px] pt-1 pb-1 rounded-lg md:hover:shadow-[0_0_12px_0_rgba(0,0,0,0.3)] md:hover:shadow-accent_pink">
                    {maker.nickName ? maker.nickName.toLowerCase() : maker.type.toLowerCase()} (Type: {maker.type})
                    <button onClick={() => deleteSelectedMaker(index)}>Delete</button>
                </div>
            ))}
        </div>
    );
